...
class pool:
    ...
    @classmethod
    def change_thread_num(cls, num, gqueue='v'):
        """
        #==============================================================
        ...
        #==============================================================
        """
        if gqueue in cls._pool_func_num:
            x = cls._pool_func_num[gqueue] - num
            # 当前线程数少于最后一次定义的数量时候会增加伺服线程
            # 多了则会杀掉多余线程
            if x < 0:
                cls._run(abs(x), gqueue)
            if x > 0:
                for _ in range(abs(x)):
                    cls._pool_queue[gqueue].put(KillThreadParams)
            cls._pool_func_num[gqueue] = num

    @classmethod
    def _run(cls, num, gqueue):
        """
        #==============================================================
        ...
        def _pools_pull():
            ...
            while True:
                v = cls._pool_queue[gqueue].get()
                if v == KillThreadParams: return
                try:
                    func, args, kw = v
                    cls._monitor_run_num[gqueue].put('V')  # 标记线程是否执行完毕
                    func(*args, **kw)
                except BaseException as e:
                    if log_flag._elog:
                        print(traceback.format_exc())
                finally:
                    cls._monitor_run_num[gqueue].get('V')  # 标记线程是否执行完毕

        # 线程的开启
        for _ in range(num): Thread(target=_pools_pull).start()

    @classmethod
    def main_monitor(cls):
        """
        #==============================================================
        # 对主线程进行监视的函数
        ...
        def _func():
            while True:
                time.sleep(.25)
                if not main_thread().isAlive() and all(map(lambda i: i.empty(), cls._monitor_run_num.values())):
                    cls.close_all()
                    break

        if not cls._monitor:
            cls._monitor = Thread(target=_func, name="MainMonitor")
            cls._monitor.start()
...
def unpatch_all(can_be_re_patch=False):
    """
    #==============================================================
    # 去补丁函数
