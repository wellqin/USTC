"""
there must be a table of encoding/decoding bases in `base64`.
...
"""
The Base85 functions use an expanded alphabet 
that is more space-efficient than the one used for Base64 encoding

Several Base85 encodings and variations are used in Mercurial, git, and the PDF file
format. 

Python includes two implementations, b85encode() implements the version used in Git Mercurial, 
and a85encode() implements the Ascii85 variant used by PDF files.
