...
that both sides of the connection know which order the bytes are in and how to assemble
them back into the correct order for the local architecture. 

The server program uses the same Struct specifier to unpack the bytes it receives 
so they are interpreted in the correct order.
"""
