...
init_() { #? Collect needed information and set options before startig main loop
  ...
  if [[ -n $curled ]]; then
    if ! get_value -v git_version -ss "$(
      curl -m 2 --raw -r 0-3500
      https://raw.githubusercontent.com/aristocratos/bashtop/master/bashtop 2>/dev/null
    )" -k "version=" -r "[^0-9.]"; then unset git_version; fi
  fi
...
color_init_() { #? Check for theme file and set colors
  ...
  for color_name in ${convert_color[@]}; do
    if [[ -n $sourced ]]; then
      hex2rgb="${theme[${color_name}]}"
    else hex2rgb="${!color_name}"; fi

    hex2rgb=${hex2rgb//#/}

    if [[ ${#hex2rgb} == 6 ]] && is_hex "$hex2rgb"; then
      hex2rgb="$((${hex}${hex2rgb:0:2})) $((${hex}${hex2rgb:2:2})) $((${hex}${hex2rgb:4:2}))"
    elif [[ ${#hex2rgb} == 2 ]] && is_hex "$hex2rgb"; then
      hex2rgb="$((${hex}${hex2rgb:0:2})) $((${hex}${hex2rgb:0:2})) $((${hex}${hex2rgb:0:2}))"
    else
      dec_test=(${hex2rgb})
      if [[ ${#dec_test[@]} -eq 3 ]] && is_int "${dec_test[@]}"; then
        hex2rgb="${dec_test[*]}"
  ...
  if [[ -n ${theme[main_bg]} ]]; then
    theme[main_bg_dec]="${theme[main_bg]}"
    theme[main_bg]=";48;2;${theme[main_bg]// /;}"
  fi
  ...
  for array_name in "temp" "cpu" "upload" "download" "used" "available" "cached" "free"; do
    ...
    rgb[red]=${rgb_start[0]}
    rgb[green]=${rgb_start[1]}
    rgb[blue]=${rgb_start[2]}
...
floating_humanizer() { #? Convert integer to floating point and scale up in steps of 1024 to highest positive unit
  ...
  until (($# == 0)); do
    case "$1" in
    -b | -bit)
      unit=(bit Kib Mib Gib Tib Pib)
      unit_mult=8
      ;;
    -B | -Byte)
      unit=(Byte KiB MiB GiB TiB PiB)
      unit_mult=1
      ;;
  ...
  until (($# == 0)); do
    until (($# == 0)); do
      ...
      -k | -key)
        key="$2"
        shift
        ;; #? Key "string" on the same line as target value
      -m | -match)
        match="$2"
        shift
        ;; #? If multiple matches on a line, match occurrence "x"
      -mk | -match-key)
        match_key=$2
        line_pos=0
        shift
        ;; #? Match in relation to key position, -1 for previous value, 1 for next value
      -b | -break)
        shift
        break
        ;; #? Break up arguments for multiple searches
      -a | -all) all=1 ;; #? Prints back found line including key
      -l | -line)
        line_nr="$2"
        shift
        ;; #? Set target line if no key is available
      -ss | -source-string)
        input="$2"
        shift
        ;; #? Argument string as source
      -sf | -source-file)
        input="$(<"$2")"
        shift
        ;; #? File as source
      ...
      -math)
        math="$2"
        shift
        ;; #? Perform math on a integer value, "x" represents value, only works if "integer" argument is given
      -i | -integer)
        reg="[\-]?[0-9]+[.,]?[0-9]*"
        int=1
        match=1
        ;; #? Match integer value or float and convert to int
      -r | -remove)
        remove+=("$2")
        shift
        ;; #? Format output by removing entered regex, can be used multiple times
      -v | -variable-out)
        local -n found="$2"
        ext_var=1
        shift
...
create_box() { #? Draw a box with an optional title at given location
  ...
  until (($# == 0)); do
    ...
    -f | -full)
      ...
      width=$((tty_width))
      height=$((tty_height))
      ;; #? Use full terminal size for box
    -c | -col) if is_int "$2"; then
      col=$2
      shift
    fi ;; #? Column position to start box
    -l | -line) if is_int "$2"; then
      line=$2
      shift
    fi ;; #? Line position to start box
    -w | -width) if is_int "$2"; then
      width=$2
      shift
    fi ;; #? Width of box
    -h | -height) if is_int "$2"; then
      height=$2
      shift
    fi ;; #? Height of box
    -t | -title) if [[ -n $2 ]]; then
      title="$2"
      shift
    fi ;; #? Draw title without titlebar
...
create_meter() { #? Create a horizontal percentage meter, usage; create_meter <value 0-100>
  ...
  until (($# == 0)); do
    case $1 in
    -p | -place) if is_int "${@:2:2}"; then
      line=$2
      col=$3
      shift 2
    fi ;; #? Placement for meter
    -w | -width)
      width=$2
      shift
      ;; #? Width of meter in columns
    -c | -color)
      local -n colors=$2
      shift
      ;; #? Name of an array containing colors from index 0-100
  ...
  if [[ -n $line && -n $col ]]; then
    print -v meter_var -rs -m $line $col
  ...
  for ((i = 1; i <= width; i++)); do
    ...
    if ((val >= i * 100 / width)); then
      ...
      print -v meter_var -fg $bg_color -rp $((1 + width - i)) -t "${block}"
      break
    else
      if [[ -n $invert ]]; then
        break
        print -v meter_var -l $((1 + width - i))
      else
        print -v meter_var -r $((1 + width - i))
        break
      fi
    fi
  done
...
create_graph() { #? Create a graph from an array of percentage values, usage; 	create_graph <options> <value-array>
  ...
  until (($# == 0)); do
    case $1 in
    -d | -dimensions) if is_int "${@:2:4}"; then
      line=$2
      col=$3
      height=$4
      width=$5
    ...
    -o | -output-array)
      ...
      ext_var=1
      shift
      ;; #? Output meter to an array
    -add-value) if is_int "$3"; then
      local -n output_array=$2
      add=$3
      break
    else return; fi ;; #? Add a value to existing graph
    -add-last)
      local -n output_array=$2
      local -n add_array=$3
      add=${add_array[-1]}
      break
      ;; #? Add last value from array to existing graph
    -i | -invert) invert=1 ;; #? Invert graph, drawing from top to bottom
    -n | -no-guide) no_guide=1 ;; #? Don't print side and bottom guide lines
    -max) if is_int "$2"; then
      max=$2
      shift
    fi ;; #? Needed max value for non percentage arrays
    *)
      local -n tmp_in_array=$1
      input_array=("${tmp_in_array[@]}")
      ;;
    esac
    shift
  done
...
create_mini_graph() { #? Create a one line high graph from an array of percentage values, usage; 	create_mini_graph <options> <value-array>
  ...
  until (($# == 0)); do
    ...
    -add-last)
      local -n output_var=$2 add_array=$3
      add="${add_array[-1]}"
      break
      ;; #? Add last value from array to existing graph
    -i | -invert) invert=1 ;; #? Invert graph, drawing from top to bottom
  ...
  #* Print spaces to right-justify graph if number of values is less than graph width
  if [[ -z $add && -z $no_color ]] && ((value_width < width)); then
    print -v graph_var -rp $((width - value_width)) -t "\e[1C"
  ...
  while ((x < value_width)); do
    ...
    if ((org_value == 100)); then
      cur_value=10
    elif [[ ${#org_value} -gt 1 && ${org_value:(-1)} -ge 5 ]]; then
      cur_value=$((${org_value::1} + 1))
    elif [[ ${#org_value} -gt 1 && ${org_value:(-1)} -lt 5 ]]; then
      cur_value=$((${org_value::1}))
    elif [[ ${org_value:(-1)} -ge 5 ]]; then
      cur_value=1
    else
      cur_value=0
    fi
    ...
    until (($# == 0)); do
      ...
      -fg | -foreground) #? Set text foreground color, accepts either 6 digit hexadecimal "#RRGGBB", 2 digit hex (greyscale) or decimal RGB "<0-255> <0-255> <0-255>"
        ...
        if is_int "${@:2:3}"; then
          fgc="\e[38;2;$2;$3;$4m"
          shift 3
        elif [[ ${#val} == 6 ]] && is_hex "$val"; then
          fgc="\e[38;2;$((${hex}${val:0:2}));$((${hex}${val:2:2}));$((${hex}${val:4:2}))m"
          shift
        elif [[ ${#val} == 2 ]] && is_hex "$val"; then
          fgc="\e[38;2;$((${hex}${val:0:2}));$((${hex}${val:0:2}));$((${hex}${val:0:2}))m"
          shift
        fi
        ;;
      -bg | -background) #? Set text background color, accepts either 6 digit hexadecimal "#RRGGBB", 2 digit hex (greyscale) or decimal RGB "<0-255> <0-255> <0-255>"
        ...
        if is_int "${@:2:3}"; then
          ...
          bgc="\e[48;2;$((${hex}${val:0:2}));$((${hex}${val:2:2}));$((${hex}${val:4:2}))m"
          shift
        elif [[ ${#val} == 2 ]] && is_hex "$val"; then
          bgc="\e[48;2;$((${hex}${val:0:2}));$((${hex}${val:0:2}));$((${hex}${val:0:2}))m"
          shift
        fi
        ;;
      ...
      -f | -font) if [[ $2 =~ ^(sans-serif|script|fraktur|monospace|double-struck)$ ]]; then
        custom_font="$2"
        shift
      fi ;; #? Set custom font
      -m | -move) if is_int "${@:2:2}"; then
        add_command="${add_command}\e[${2};${3}f"
        shift 2
      fi ;; #? Move to postion "LINE" "COLUMN"
      -l | -left) if is_int "$2"; then
        add_command="${add_command}\e[${2}D"
        shift
      fi ;; #? Move left x columns
      -r | -right) if is_int "$2"; then
        add_command="${add_command}\e[${2}C"
        shift
      fi ;; #? Move right x columns
      -u | -up) if is_int "$2"; then
        add_command="${add_command}\e[${2}A"
        shift
      fi ;; #? Move up x lines
      -d | -down) if is_int "$2"; then
        add_command="${add_command}\e[${2}B"
        shift
      fi ;; #? Move down x lines
      -jl | -justify-left) if is_int "$2"; then
        justify_left="${2}"
        shift
      fi ;; #? Justify string left within given width
      -jr | -justify-right) if is_int "$2"; then
        justify_right="${2}"
        shift
      fi ;; #? Justify string right within given width
      -jc | -justify-center) if is_int "$2"; then
        justify_center="${2}"
        shift
      fi ;; #? Justify string center within given width
      -rp | -repeat) if is_int "$2"; then
        repeat=${2}
        shift
      fi ;; #? Repeat next string x number of times
      ...
      -v | -variable)
        ...
        ext_var=1
        shift
        ;; #? Send output to a variable, appending if not unset
      *)
        text="$1"
        shift
        break
        ;; #? Assumes text string if no argument is found
      esac
      shift
    done
...
collect_cpu() { #? Collects cpu stats from /proc/stat and compares with previously collected sample to get cpu usage
  ...
  if ((${#cpu[freq]} > 3)); then
    cpu[freq_string]="${cpu[freq]::-3}.${cpu[freq]:(-3):1} GHz"
  elif ((${#cpu[freq]} > 1)); then
    cpu[freq_string]="${cpu[freq]} MHz"
...
collect_cpu_temps() { #? Collect cpu temperatures
  ...
  if get_value -v cpu[temp_0] -sv "sens_var" -k "Package*:" -mk 1; then
    ...
    cpu[temp_unit]=${cpu[temp_0]:(-2)}
    cpu[temp_0]=${cpu[temp_0]%.*}
    if [[ ${cpu[temp_0]::1} == "+" ]]; then cpu[temp_0]=${cpu[temp_0]#+}; fi
...
collect_processes() { #? Collect process information and calculate accurate cpu usage
  ...
  if ((width > 60)); then
    format_args=",args:$((width - (47 + proc[pid_len])))=Arguments:"
    format_cmd=15
      ...
      if [[ -n ${proc[old_${pid}_ticks]} ]]; then
        ...
        if [[ ${pid} == "${proc[detailed_pid]}" ]]; then
          ...
          if [[ ${proc[detailed_mem]} != "${work_array[-2]}" || -n $get_mem ]] || ((++proc[detailed_mem_count] > 5)); then
            ...
            if ((proc[detailed_mem_int] > 900)); then
              proc[detailed_mem_int]=$((proc[detailed_mem_int] / 10))
            elif ((proc[detailed_mem_int] > 600)); then
              proc[detailed_mem_int]=$((proc[detailed_mem_int] / 8))
            elif ((proc[detailed_mem_int] > 300)); then
              proc[detailed_mem_int]=$((proc[detailed_mem_int] / 5))
            elif ((proc[detailed_mem_int] > 100)); then
              proc[detailed_mem_int]=$((proc[detailed_mem_int] / 2))
        ...
        tmp_percent="${proc_array[count]:(-5)}"
        tmp_percent="${tmp_percent// /}"
        if [[ ${tmp_percent//./} != "$tmp_percent" ]]; then tmp_percent="${tmp_percent::-2}"; fi
  ...
  if ((proc[detailed] == 1)) && [[ -z ${proc[detailed_cpu]} && -z ${proc[detailed_killed]} ]]; then
    proc[detailed_killed]=1
...
calc_sizes() { #? Calculate width and height of all boxes
  ...
  if ((threads > (cpu_height - 3) * 3 && tty_width >= 200)); then
    box[p_width]=$((24 * 4))
    box[p_height]=$((threads / 4 + 4))
    box_cols=4
  elif ((threads > (cpu_height - 3) * 2 && tty_width >= 150)); then
    box[p_width]=$((24 * 3))
    box[p_height]=$((threads / 3 + 5))
    box_cols=3
  elif ((threads > cpu_height - 3 && tty_width >= 100)); then
    box[p_width]=$((24 * 2))
    box[p_height]=$((threads / 2 + 4))
    box_cols=2
  else
    box[p_width]=24
    box[p_height]=$((threads + 4))
    box_cols=1
  fi
...
draw_cpu() { #? Draw cpu and core graphs and print percentages
  ...
  if ((resized > 0)); then
    ...
    graph_a_size=$((height / 2))
    graph_b_size=${graph_a_size}
  ...
  for ((i = 0; i <= threads; i++)); do
    ...
    if ((i == 0 & p_width > 24 + temp_width)); then
      name="CPU Total "
      meter_width=$((p_width - 17 - temp_width))
    fi
    ...
    if ((p_width > 84 + temp_width & i >= (p_height - 2) * 3 - 2)); then
      pt_line=$((p_line + i - y * 4))
      pt_col=$((p_col + 72 + temp_width * 3))
    elif ((p_width > 54 + temp_width & i >= (p_height - 2) * 2 - 1)); then
      pt_line=$((p_line + i - y * 3))
      pt_col=$((p_col + 48 + temp_width * 2))
    elif ((p_width > 24 + temp_width & i >= p_height - 2)); then
      pt_line=$((p_line + i - y * 2))
      pt_col=$((p_col + 24 + temp_width))
    else y=$i; fi
...
draw_mem() { #? Draw mem, swap and disk statistics
  ...
  for type in ${types[@]}; do
    ...
    for value in "used" "available" "cached" "free"; do
      if [[ $type == "swap" && $value == "available" ]]; then
        value="free"
...
draw_processes() { #? Draw processes and values to screen
  ...
  if ((proc[detailed_change] > 0)) || ((proc[detailed] > 0 & resized > 0)); then
    ...
    if ((proc[detailed] == 1)); then
      ...
      if [[ ${#detail_history[@]} -eq 1 ]] || ((resized > 0)); then
        ...
        if ((${#proc[detailed_cmd]} > (right_width - 6) * 2)); then
          ((det_y--))
          dets=2
      ...
      if ((proc[selected] > 0)); then
        enter_fg="${theme[inactive_fg]}"
        enter_a_fg="${theme[inactive_fg]}"
      else
        enter_fg="${theme[title]}"
        enter_a_fg="${theme[hi_fg]}"
      fi
      if [[ -n ${proc[detailed_killed]} ]]; then
        misc_fg="${theme[title]}"
        misc_a_fg="${theme[hi_fg]}"
      else
        misc_fg=$enter_fg
        misc_a_fg=$enter_a_fg
      fi
  ...
  if [[ ${proc[detailed]} -eq 1 ]]; then
    ...
    if [[ -n ${proc[detailed_killed]} ]]; then
      det_status="stopped"
      status_color="${theme[inactive_fg]}"
    else
      det_status="running"
      status_color="${theme[proc_misc]}"
    fi
  ...
  for out_line in "${proc_array[@]:$proc_start}"; do
    pid="${out_line::$((proc[pid_len] + 1))}"
    pid="${pid// /}"
    pid_graph="pid_${pid}_graph"

    if ((current_num == proc[selected])); then
      print -v proc_out -bg ${theme[selected_bg]} -fg ${theme[selected_fg]} -b
      proc[selected_pid]="$pid"
    ...
    if ((current_num < proc[selected] + 1)); then
      fg_step_r=$((fg_step_r - fg_add_r))
      fg_step_g=$((fg_step_g - fg_add_g))
      fg_step_b=$((fg_step_b - fg_add_b))
      pid_step_r=$((pid_step_r - pid_add_r))
      pid_step_g=$((pid_step_g - pid_add_g))
      pid_step_b=$((pid_step_b - pid_add_b))
  ...
  if ((proc[order_change] == 1 | proc[filter_change] == 1 | resized > 0)); then
    ...
    if [[ -z $filter && -z $input_to_filter ]]; then
      ...
      if [[ ${#filter} -le $((width - 35 - reverse_pos)) ]]; then
        filter_string="${filter}"
      elif [[ ${#filter} -gt $((width - 35 - reverse_pos)) ]]; then
      ...
      if [[ ${#filter} -le $((width - 35 - reverse_pos - 4)) ]]; then
        filter_string="${filter}"
      elif [[ ${#filter} -gt $((width - 35 - reverse_pos - 4)) ]]; then
        filter_string="${filter::$((width - 35 - reverse_pos - 4))}"
      fi
  ...
  if ((proc[page_change] == 1 | resized > 0)); then
    ...
    if ((proc[selected] > 0)); then
      ...
      up_fg="${theme[inactive_fg]}"
      kill_fg="${theme[inactive_fg]}"
      com_fg="${theme[inactive_fg]}"
    fi
    if ((proc[selected] == ${#proc_array[@]} - proc_start)); then down_fg="${theme[inactive_fg]}"; else down_fg="${theme[hi_fg]}"; fi

    if ((proc[page] > 1)); then
      page_up_fg="${theme[title]}"
      pg_arrow_up_fg="${theme[hi_fg]}"
    else
      page_up_fg="${theme[inactive_fg]}"
      pg_arrow_up_fg="${theme[inactive_fg]}"
    fi
    if ((proc[page] < proc[pages])); then
      page_down_fg="${theme[title]}"
      pg_arrow_down_fg="${theme[hi_fg]}"
    else
      page_down_fg="${theme[inactive_fg]}"
      pg_arrow_down_fg="${theme[inactive_fg]}"
    fi
...
draw_clock() { #? Draw a clock at top of screen
  ...
  if [[ $resized -gt 0 && $resized -lt 5 ]]; then
    unset clock_out
    return
  fi
...
menu_() { #? Shows the main menu overlay
  ...
  until false; do
    ...
    if [[ -n ${keypress} || -z ${menu_out} ]]; then
      ...
      unset up
      if [[ -n ${theme[main_bg_dec]} ]] && ((${theme[main_bg_dec]// /*} > 255 ** 3 / 2)); then
        print -v menu_out -bg "#00"
        unset trans
      else trans=" -trans"; fi
    ...
    if ((time_left > 1000)); then
      wait_string=1
      time_left=$((time_left - 1000))
    elif ((time_left > 1)); then
...
options_() { #? Shows the options overlay
  ...
  desc_color_theme=("Set bashtop color theme."
    ...
    " ")
  if [[ -z $curled ]]; then
    desc_color_theme+=("Get more themes at:"
  ...
  until false; do
    ...
    if [[ -n $keypress || -z $options_out ]]; then
      ...
      if [[ $background_update == false ]]; then
        desc_pos=$line
        desc_height=$((${#options_array[@]} * 2 + 2))
      elif ((selected_int * 2 + ${#selected_desc[@]} < ${#options_array[@]} * 2)); then
        desc_pos=$((line + selected_int * 2))
      ...
      for ((i = 0, ypos = 1; i < ${#options_array[@]}; i++, ypos = ypos + 2)); do
        ...
        if [[ -n $inputting && ${option_string} == "${selected}" ]]; then
          if [[ ${#inputting_value} -gt 14 ]]; then
            option_value="${inputting_value:(-14)}_"
    ...
    if [[ -n $valid ]]; then
      ...
      "update_ms enter")
        if [[ -z $inputting ]]; then
          inputting=1
          inputting_value="${update_ms}"
        else
          if ((inputting_value < 86400000)); then
            update_ms="${inputting_value:-0}"
            updated_ms=1
          fi
      ...
      "draw_clock enter")
        if [[ -z $inputting ]]; then
          inputting=1
          inputting_value="${draw_clock}"
        else
          draw_clock="${inputting_value}"
          unset inputting inputting_value clock_out
        fi
        ;;
      "custom_cpu_name enter")
        if [[ -z $inputting ]]; then
          inputting=1
          inputting_value="${custom_cpu_name}"
        else
          custom_cpu_name="${inputting_value}"
          changed_cpu_name=1
          unset inputting inputting_value
        fi
        ;;
      "check_temp"* | "error_logging"* | "background_update"*)
        ...
        if [[ ${selected_var} == "true" ]]; then
          selected_var="false"
          if [[ $selected == "proc_reversed" ]]; then
            proc[order_change]=1
            unset 'proc[reverse]'
          fi
        else
        ...
        ;;
      "proc_sorting right")
        if ((proc[sorting_int] < ${#sorting[@]} - 1)); then
          ((++proc[sorting_int]))
...
get_key() { #? Get one key from standard input and translate key code to readable format
  ...
  until (($# == 0)); do
    ...
    -w | -wait)
      wait_time="$2"
      shift
      ;; #* Time to wait for key
    -s | -save) save=1 ;; #* Save key for later processing
    esac
    shift
  done

  if [[ -z $save && -n ${saved_key[0]} ]]; then
    key="${saved_key[0]}"
    unset 'saved_key[0]'
    saved_key=("${saved_key[@]}")
  else
    ...
    if [[ $key == "${enter_key}" ]]; then
      ...
      esc=1
      read -rsn3 -t 0.0001 key || true
    fi
    if [[ -z $key && $esc -eq 1 ]]; then
      key="escape"
    elif [[ $esc -eq 1 ]]; then
      case "${key}" in
      '[A') key="up" ;;
    ...
    *) if [[ ${#keypress} -eq 1 ]]; then filter+="${keypress//[\\\$\"\']/}"; else unset filter_change; fi ;;
    ...
    up | shift_tab) #* Move process selector up one
      if [[ ${proc[selected]} -gt 0 ]]; then
        ((proc[selected]--))
        if ((proc[page] > 1 & proc[selected] == 0)); then
          ((--proc[page]))
    ...
    down | tab) #* Move process selector down one
      if ! ((proc[page] == proc[pages] & proc[selected] >= box[processes_height] - 3)); then
        ...
        if ((proc[selected] > box[processes_height] - 3)); then
          ((proc[page]++))
          proc[selected]=1
        fi
        proc[page_change]=1
...
main_loop() { #? main loop...
  ...
  if ((resized >= 5)); then
    ...
    unset draw_out proc_out clock_out
    return
  fi
...
if [[ $error_logging == true ]]; then
  ...
  (
    echo " "
    echo "New instance of bashtop version: ${version} Pid: $$"
  ) >>"${config_dir}/error.log"
