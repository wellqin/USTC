...
"""
# 为了美观不报错，虚构值, 假设EXPR = list(),并虚构yieldFrom、yieldFromSolution二个函数
EXPR = list()
...
3. 调用方让子生成器自己抛出异常
4. 当调用方使用next()或者.send(None)时，都要在子生成器上调用next()函数，当调用方使用.send()发送非 None 值时，
   才调用子生成器的.send()方法；
"""


def yieldFromSolution():
    _i = iter(EXPR)
    try:
        _y = next(_i)
    except StopIteration as _e:
